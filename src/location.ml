...
type span =
  | Span of Lexing.position * Lexing.position
...
type 'a t = { data : 'a; loc : span } [@@deriving show { with_path = false }]

let make_span start finish = Span (start, finish)

let span_of_lexbuf lexbuf =
  make_span (Lexing.lexeme_start_p lexbuf) (Lexing.lexeme_end_p lexbuf)

let locate ?(s = Nowhere) x = { data = x; loc = s }
